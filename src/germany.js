...
// Data is loaded on demand from sibling files (see scripts/build-germany-data.mjs):
//   germany/states.json        – borders + counts (once, up front)
//   germany/hills/<slug>.json   – a state's hills (on click)
//   germany/arms/<slug>.png     – a state's coat of arms (on click)
...
// point in (screen-space) polygon, with holes
...
function renderOverview() {
  ...
  ctx.fillText("ГОРЫ И ХОЛМЫ ГЕРМАНИИ", MAP_PAD_X, 70);
  ctx.font = "italic 22px Georgia, serif";
  ctx.fillStyle = "#555";
  ctx.fillText("выберите федеральную землю — кликните по карте", MAP_PAD_X, 104);
...
async function enterDetail(slug) {
  ...
  // draw outline immediately while the hills load
  ...
  // user may have gone back / picked another state while loading
...
function renderDetail(minEle) {
  ...
  let budget = 90; // cap labels so the densest states stay readable
...
saveBtn.addEventListener("click", () => {
  canvas.toBlob((blob) => {
    ...
    a.href = url;
    a.download = `germany-hills-${detailState ? detailState.slug : "map"}-${slider.value}m.png`;
    a.click();
